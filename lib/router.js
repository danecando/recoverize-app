// Subscription Caching
var subs = new SubsManager()


// Router wide configuration
Router.configure({
    layoutTemplate: 'layout',
    notFoundTemplate: 'notFound',
    loadingTemplate: 'loading',

    // wait on the following subscriptions before rendering the page to ensure
    // the data it's expecting is present
    waitOn: function() {
        return [
            Meteor.subscribe('userData'),
            Meteor.subscribe('notification'),
            Meteor.subscribe('chat'),
            Meteor.subscribe('presence')
        ]
    }
})

// Permit certain routes for users not signed in
Router.onBeforeAction(function() {

    if(Meteor.isClient){
        // trailing slash safety check
        var path = this.location.get().path
        if(path[path.length-1] !== '/') path += '/'

        // toggle resolved notifications
        Notification.find({path: path, checked: false}).forEach(function (notification) {
            Meteor.call('checkNotification', notification._id)
        })
    }

    if(Meteor.isClient){
        if(!Meteor.userId()){
            this.render('signin')
            this.redirect('/signin')
        }else{
            this.next()
        }
    }else{
        this.next()
    }

    // these routes are open to non registered users
}, { except: ['join', 'help'] })


///////////////////////////////////////////////////
// Client side routes
///////////////////////////////////////////////////

// Auth routes
Router.route('join')
Router.route('signin')

// App routes
Router.route('/', {
    action: function() {
        this.render('dashboard')
    }
})

Router.route('/chat')
Router.route('/notifications')

Router.route('/messages', {
    onBeforeAction: function() {


        this.next()
    },
    waitOn: function() {

        // todo: why does this get called many times? (dont want to .find() repeatedly)
        function getOtherMember(member1, member2){
            return member1 === Meteor.user().username ? member2 : member1
        }

        var usernames = MessageSessions.find().map(function(session) {
            var otherMember = getOtherMember(session.members[0], session.members[1])
            return otherMember
        })

        return [
            Meteor.subscribe('message'),
            Meteor.subscribe('messagesWith', usernames)
        ]
    },
    action: function(){
        this.render('messages')
    }
})

Router.route('/messages/:username', {
    waitOn: function(){
        return Meteor.subscribe('message', this.params.username)
    },
    action: function(){
        this.render('message', {
            data: {
                username: this.params.username
            }
        })
    }
})

// User routes
Router.route('/profile', {
    action: function() {
        this.render('profileUpdate')
    }
})

Router.route('/account', {
    action: function() {
        this.render('accountSettings')
    }
})

Router.route('/logout', {
    action: function() {
        Meteor.logout()
        this.render('join')
    }
})

// public user profiles
Router.route('/users/:username', {
    waitOn: function(){
        return Meteor.subscribe('userPublic', this.params.username)
    },
    action: function(){
        this.render('user', {
            data: {
                username: this.params.username
            }
        })
    }
})

///////////////////////////////////////////////////
// SERVER SIDE ROUTES
// are kind of buggy!!!
///////////////////////////////////////////////////

//Router.route('/profile/update', { where: 'server' })
//    .get(function () {
//        this.render('profileUpdate');
//    })
//    .post(function () {
//        this.response.end('called');
//    });