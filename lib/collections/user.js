            ...
            if (user.email !== Meteor.user().emails[0].address) {
                Meteor.users.update({ _id: Meteor.userId() }, {
                    ...
                    if (!error) { // if success remove old email address
                        ...
                        throw new Meteor.Error(500, "Couldn't update your email address at this time")
                    }
                })
            }
        },
        ...
        createUsername: function(username) {
            if (!Meteor.users.findOne({ username: username })) {
                Meteor.users.update({_id: Meteor.userId()}, {
                    ...
                    if (error) throw new Meteor.Error("Username can only contain letters & numbers")
                })
            } else {
                throw new Meteor.Error("This username already exists")
            }
        },
