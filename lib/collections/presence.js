...
// todo: refactor possibly as a plugin make more efficient

if (Meteor.isServer) {
  var connections = {};

  var expire = function(id) {
    var presence = Presences.findOne({ _id: id });
    ...
    if (presence) {
      ...
      Presences.remove({ _id: id }, function(err) {
  ...
  Meteor.onConnection(function(connection) {
    Presences.insert({ _id: connection.id }, function(err) {
      // async
    });
